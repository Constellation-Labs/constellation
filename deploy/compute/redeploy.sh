...
sbt assembly && \
pssh -h $HOSTS_FILE -O StrictHostKeyChecking=no -i 'sudo apt install -y google-cloud-sdk' # Snapshot GCP image didn't have this for some reason, remove later
...
pssh -h $HOSTS_FILE -O StrictHostKeyChecking=no -i "gsutil cp gs://constellation-dag/release/dag-$JAR_TAG.jar ." && \

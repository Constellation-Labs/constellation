package org.constellation.checkpoint.validator

import java.security.KeyPair

import cats.effect.{ContextShift, IO}
import org.constellation.domain.blacklist.BlacklistedAddresses
import org.constellation.domain.checkpointBlock.CheckpointBlockDoubleSpendChecker
...
class CheckpointBlockDoubleSpendCheckerTest extends FunSuite with BeforeAndAfter with Matchers {
  ...
  test("it should return transaction when transaction's ordinal number is less than or equal the last accepted one") {
    ...
    val secondTransaction = createTransaction("src", firstTransaction.baseHash, firstTransaction.ordinal)
    ...
    val transactionToValidate = createTransaction("src", firstTransaction.baseHash, firstTransaction.ordinal)

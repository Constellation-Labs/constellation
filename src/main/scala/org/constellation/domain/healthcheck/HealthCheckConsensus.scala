...
import org.constellation.domain.healthcheck.HealthCheckConsensusManager.HistoricalRound
...
class HealthCheckConsensus[F[_]](
  ...
  val fetchProposalAfter = 120L
  val removeUnresponsivePeersWithParallelRoundAfter = 60L
  ...
  private[healthcheck] val roundPeers: Ref[F, Map[Id, RoundData]] = {
    Ref.unsafe(
      initialRoundPeers
        .mapValues(pd => RoundData(pd.asRight[ProxyPeers]))
    )
  }
  ...
  def removePeer(id: Id): F[Unit] = roundPeers.modify(peers => (peers - id, ()))
  ...
  def manageAbsentPeers(absentPeers: Set[Id]): F[Unit] =
    for {
      ...
      _ <- toRemove.toList.traverse(removePeer)
    } yield ()
  ...
  def removeIdAsAProxy(proxyingId: Id, proxyingToId: Id): F[Unit] =
    roundPeers.modify { peers =>
      val updatedPeer = peers.get(proxyingToId).flatMap { roundData =>
      ...
      updatedPeer match {
        case Some(roundData) => (peers + (proxyingToId -> roundData), ())
        case None            => (peers - proxyingToId, ())
      }
    }
  ...
  def generateHistoricalRoundData(healtcheckDecision: Option[HealthcheckConsensusDecision]): F[HistoricalRound] =
    for {
  ...
  def processProposal(healthProposal: ConsensusHealthStatus): F[Unit] =
    for {
      ...
      initiallyUnknownPeers = healthProposal.clusterState.keySet.diff(initialRoundPeers.keySet).filterNot(_ == ownId) // I think we don't need to filter out peers that left because they will not be added here by mistake
      data <- roundPeers.modify { roundPeers =>
  ...
  private def fetchProposals(): F[Unit] =
    for {
      ...
      _ <- if (isAcquired)
        fetchHealthProposals()
          .handleErrorWith(
            e =>
              fetchingProposalLock.release >> logger
                .error(e)("Error fetching healthcheck proposal.") /*>> e.raiseError[F, Unit]*/
          ) //not needed I think because every fetch failure is handled in fetchHealthProposals, should do the same for sendHealthProposal I think
      else
        F.unit
      _ <- fetchingProposalLock.release
    } yield ()
  ...
  private def fetchHealthProposals(): F[Unit] =
    for {
            ...
            case Left(proxyPeers) =>
              for {
                ...
                response <- PeerResponse.run(
                  ...
                  unboundedBlocker
                )(proxyPeer.peerMetadata.toPeerClientMetadata.copy(port = "9003"))
              } yield response
            case Right(peerData) =>
              PeerResponse.run(
      ...
      _ <- succeeded.traverse { case (_, proposal) => processProposal(proposal.consensusHealthStatus) }
      _ <- failed.traverse {
        case (id, error) =>
          logger.debug(error)(s"Error during fetching healtcheck proposal from peer with id=$id.")
      }
    } yield ()
    ...
    for {
      ...
      _ <- if (isAcquired)
        sendHealthStatus()
          .handleErrorWith(
            e =>
              sendingProposalLock.release >> logger
                .error(e)("Error sending healtcheck proposal") /*>> e.raiseError[F, Unit]*/
          )
      else
        F.unit
      _ <- sendingProposalLock.release
    } yield ()
  ...
  private def sendHealthStatus(): F[Unit] =
    for {
      ...
      succeeded <- peers.toList.traverse {
        case (id, roundData) => {
          roundData.peerData match {
            case Left(proxyPeers) =>
              for {
        ...
        }.map(result => if (result.isRight) id.some else None)
      }.map(_.flatten)
  ...
  def isStatusOnline(status: PeerHealthCheckStatus): Boolean =
    status match {
      ...
      case PeerUnresponsive(_) | UnknownPeer(_) => false
    }

  // remember that we also need a logic to remove proxies for peer if they no longer have a connection to the peer(check response and if the response says they don't have that node
  // anymore then just remove the proxy node from proxyPeers list)
  def calculateConsensusOutcome(): F[Option[HealthcheckConsensusDecision]] =
    for {
      ...
      _ <- logger.debug(s"calculateConsensusOutcome for roundIds: $roundIds is $result")
    } yield result
...
object HealthCheckConsensus {
  ...
  case class RoundState(
    parallelRounds: Set[Set[RoundId]],
    peersThatLeft: Set[Id],
    parallelRoundsOutcome: List[RoundOutcome]
  )

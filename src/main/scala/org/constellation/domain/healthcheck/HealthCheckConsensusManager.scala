...
class HealthCheckConsensusManager[F[_]](
  ...
  // maybe this could be dynamic and dependent on cluster size, the bigger the cluster size the longer timespan (but then it would need to be dynamic!)
  ...
  private[healthcheck] val peersToRunHealthcheckFor: Ref[F, Set[Id]] =
    Ref.unsafe(Set.empty) // use THIS to start healthchecks
  private[healthcheck] val peersToRunConsensusFor: Ref[F, Set[Id]] =
    Ref.unsafe(Set.empty) // use THIS to start consensuses
  private[healthcheck] val peersThatNeedReconciliation: Ref[F, Set[Id]] = Ref.unsafe(Set.empty)
  private[healthcheck] val timeOfLastOwnReconciliationRound: Ref[F, Option[FiniteDuration]] = Ref.unsafe(None)
  private[healthcheck] val runReconciliationRoundAfter: Ref[F, Option[FiniteDuration]] = Ref.unsafe(None)
    ...
    for {
        ...
        case ConsensusRounds(historical, inProgress, keepUpRounds) =>
          ...
          val (updatedKeepUpRounds, result): (
            ...
            if (isRoundInProgressForId)
              ...
              (
                keepUpRounds,
                (KeepUpRoundForGivenIdAlreadyTakingPlace(checkedId), keepUpRounds.get(checkedId), Map.empty)
              )
            else
              (
                keepUpRounds + (checkedId -> consensusRound),
                (KeepUpRoundStarted(roundIds), consensusRound.some, parallelRounds)
              )
      ...
      _ <- (response, consensusRound) match {
        ...
        case (RoundForGivenIdIsInProgress(id), None) =>
          logger.debug(
            s"Round for given id=$id while processing keepUpRound for roundIds=$roundIds turned out to be in progress."
          )
        ...
        case _ =>
          logger.warn(
            s"Unexpected case when processing keepUpRound for roundIds=$roundIds response=$response consensus=$consensusRound parallelRounds=$parallelRounds"
          )
      }
    } yield ()
  ...
  def proxySendHealthProposal(
    ...
    for {
      ...
      result <- maybePeerData match {
        case Some(peerData) =>
          logger.debug(s"Proxying healthProposal from ${consensusHealthStatus.checkingPeerId.medium} to ${proxyToPeer.medium}") >>
            PeerResponse
              .run(
  ...
  def proxyGetHealthStatusForRound(
    ...
    for {
      ...
      result <- maybePeerData match {
        case Some(peerData) =>
          logger.debug(s"Proxying getting healthProposal from ${proxyToPeer.medium}") >>
            PeerResponse
              .run(
        ...
        case None =>
          logger.debug(s"Unknown peer=$proxyToPeer to proxy getting healthProposal from.") >>
            UnknownProxyIdFetching(proxyToId = proxyToPeer, proxyingId = ownId).asLeft[SendPerceivedHealthStatus].pure[F]
      }
    } yield result
    ...
    for {
      ...
      _ <- consensuses.traverse { consensus =>
        for {
          ...
          absentPeers = (roundPeers.filterNot { case(_, rd) => rd.peerData.isLeft }.keySet -- peers.keySet) ++
            roundPeers.keySet.intersect(leavingOrOfflinePeers)
  ...
  def monitorAndManageConsensuses(consensuses: Map[Id, HealthCheckConsensus[F]]): F[Unit] =
    for {
      ...
      toRemove = peersWithDecision.filter {
        case (id, (maybeDecision, _)) => maybeDecision.exists(_.isInstanceOf[PeerOffline])
      }
      _ <- logger.debug(s"toRemove: ${toRemove.keySet.map(_.medium)}")
      toRemain = peersWithDecision.filter {
        case (id, (maybeDecision, _)) => maybeDecision.exists(_.isInstanceOf[PeerOnline])
      }
      _ <- logger.debug(s"toRemain: ${toRemain.keySet.map(_.medium)}")
      unknown = peersWithDecision.filter {
        case (id, (maybeDecision, _)) => maybeDecision.isEmpty
      }
      ...
      _ <- if (toRemainNotPresentPeers.nonEmpty)
        logger.warn(s"Peers that we don't have but we should: ${toRemainNotPresentPeers.map(_.medium)}")
      else F.unit
        ...
        case (id, (decision, _)) =>
          decision match {
            case Some(PeerOffline(id, allPeers, remainingPeers, percentage, parallelRounds, roundIds, isKeepUpRound)) =>
              logger.debug(
                s"HealthcheckConsensus decision for id=${id.medium} is: PeerOffline allPeers=${allPeers
            ...
            case Some(PeerOnline(id, allPeers, remainingPeers, percentage, parallelRounds, roundIds, isKeepUpRound)) =>
              logger.debug(
                s"HealthcheckConsensus decision for id=${id.medium} is: PeerOnline allPeers=${allPeers
                  .map(_.medium)} remainingPeers=${remainingPeers.map(_.medium)} percentage=$percentage parallelRounds=${parallelRounds
                  .map(r => r._1.medium -> r._2)} roundIds=$roundIds isKeepUpRound=$isKeepUpRound"
              )
      ...
      _ <- result match {
        case Some(ReconciliationResult(peersToRunHealthCheckFor, misalignedPeers, clusterAlignment)) =>
          for {
            ...
            _ <- logger.debug(
              s"Own[${ownId.medium}] reconciliation round with id=$roundId result: peersToRunHealthcheckFor: ${peersToRunHealthCheckFor
                .map(_.medium)} misalignedPeers: ${misalignedPeers.map(_.medium)}, clusterAlignment: ${clusterAlignment
                .map(_.toList.map { case (id, result) => id.short -> result.map {
                  case ReconciliationRound.NodeAligned => "nodeAligned"
                  case NodeNotPresentOnAllNodes(presentOn, notPresentOn) => s"presentOn=${presentOn.map(_.medium)} notPresentOn=${notPresentOn.map(_.medium)}"
                  case NodeInconsistentlySeenAsOnlineOrOffline(onlineOn, offlineOn) => s"onlineOn=${onlineOn.map(_.medium)} offlineOn=${offlineOn.map(_.medium)}"
                } })}"
            )
          } yield ()
        case None => F.unit
  ...
  def scheduleSoonEpoch(): F[FiniteDuration] =
    for {
      currentEpoch <- getTimeInSeconds()
      delayTime <- F.delay(Random.nextInt(540).seconds) // 9min * 60sec
      result = currentEpoch + 1.minute + delayTime
    } yield result

  // Schedule between 10 and 60 minutes from now
  def scheduleNormalEpoch(): F[FiniteDuration] =
    for {
      currentEpoch <- getTimeInSeconds()
      delayTime <- F.delay(Random.nextInt(3000).seconds) // 50min * 60sec
      result = currentEpoch + 10.minutes + delayTime
    } yield result
        ...
        case Some(roundEpoch) if roundEpoch < currentEpoch =>
          for {
            ...
            round <- reconciliationRound.modify { _ =>
              val round = new ReconciliationRound[F](
                ownId,
                cluster,
                currentEpoch,
                roundId,
  ...
  def checkPeersHealth(): F[Unit] =
    for {
      ...
      peersToRunConsensusFor <- peersToRunConsensusFor.get
      unresponsivePeers <- peerHealthCheck.check(peersUnderConsensus, peersToRunHealthcheckFor)
      _ <- unresponsivePeers.traverse(pd => startOwnConsensusForId(pd.peerMetadata.id))
      //_ <- peersToRunConsensusFor...
    } yield ()
...
object HealthCheckConsensusManager {
  ...
  object FetchProposalError {
    implicit val fetchProposalErrorEncoder: Encoder[FetchProposalError] = Encoder.instance {
      ...
      case a @ FetchingFailed(_)                                    => a.asJson
  ...
  object FetchingFailed {
    implicit val fetchingFailedCodec: Codec[FetchingFailed] = deriveCodec
  }
  ...
  object SendProposalError {
    implicit val sendProposalErrorEncoder: Encoder[SendProposalError] = Encoder.instance {
      case a @ SendingFailed(_)            => a.asJson
      case a @ UnknownProxyIdSending(_, _) => a.asJson
  ...
  object SendingFailed {
    implicit val sendingFailed: Codec[SendingFailed] = deriveCodec
  }

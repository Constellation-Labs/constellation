...
class HealthChecker[F[_]: Concurrent: Logger](
  ...
  def checkClusterConsistency(ownSnapshots: List[RecentSnapshot]): F[Option[List[RecentSnapshot]]] = {
    val check = for {
      ...
      result <- if (shouldReDownload(ownSnapshots, diff)) {
        startReDownload(diff, peers.filterKeys(diff.peers.contains), ownSnapshots, major)
  ...
  def startReDownload(
    ...
    major: (Seq[RecentSnapshot], Set[Id])
  ): F[Unit] = {
    val reDownload = for {
      _ <- Logger[F].info(
        s"[${dao.id.short}] Starting re-download process with : " +

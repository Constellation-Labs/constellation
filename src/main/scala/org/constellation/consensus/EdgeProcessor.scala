...
object EdgeProcessor {
  ...
  def signFlow(dao: DAO, cb: CheckpointBlock): Unit = {
    // Mock
...
// TODO: Move memPool update logic to actor and send response back to EdgeProcessor
  ...
  def handleSignatureRequest(sr: SignatureRequest)(implicit kp: KeyPair, dao: DAO): Unit = {//Todo resolve here still scatter cp if conflict, but add info about conflict
...
case class MemPool(
  ...
  def plus(checkpointBlock: CheckpointBlock, facilitatorCount: Option[Int] = None)(implicit dao: DAO): MemPool = {
    ...
    def finished: Boolean = facilitatorCount.map{//Todo validation
...
class EdgeProcessor(dao: DAO)
  ...
  def active(memPool: MemPool): Receive = {
    ...
    case HandleCheckpoint(cb) =>
      //todo, resolver, validation, conflicts, then into mempool

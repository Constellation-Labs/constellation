  ...
  private def acceptSnapshot(hash: String, rawSnapshot: Array[Byte]): F[Unit] =
    C.evalOn(ConstellationExecutionContext.unbounded) {
      (for {
        ...
        _ <- EitherT.liftF(unsafeLogger.debug(s"Accept downloaded snapshot hash=$hash at height=${snapshot.height} with blocks: ${snapshot.snapshot.checkpointBlocks}"))
...
object Download extends StrictLogging {
  ...
  def getMajoritySnapshotTest()(implicit dao: DAO, ec: ExecutionContext): Try[IO[String]] =
    tryWithMetric(
      {

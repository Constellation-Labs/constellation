...
object RandomTransactionManager {
  ...
  def generateRandomMessages()(implicit dao: DAO): Unit = {
      val cm = if (
        ...
        if (dao.threadSafeMessageMemPool.unsafeCount < 3) {
          ...
          dao.messageService.get(channel).flatMap { data =>
            if (lock.tryAcquire()) {
              Some(ChannelMessage.create(Random.nextInt(1000).toString, data.signedMessageData.signatures.hash, channel))
            } else None
          }
        } else None
      }
  ...
  def trigger(round: Long)(implicit dao: DAO): Future[Try[Any]] = {
    ...
    futureTryWithTimeoutMetric({
      ...
      if (dao.metricsManager != null && peerIds.nonEmpty && dao.nodeState == NodeState.Ready && dao.generateRandomTX) {

...
*/
    ...
    def storeResolution(db: KVDB, cache: CheckpointCacheData): Unit = {
      checkpoint.edge.storeCheckpointData(db, {prevCache: CheckpointCacheData => prevCache.copy(resolved = true)}, cache, true)
    }

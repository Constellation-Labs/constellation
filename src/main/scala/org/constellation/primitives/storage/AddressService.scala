...
class AddressService(size: Int) extends StorageService[AddressCacheData](size) {
  def transfer(src: Address, dst: Address, amount: Long): IO[AddressCacheData] =
    AddressService.locks.acquire(List(src.hash, dst.hash)) {
      updateAsync(src.hash, { a => a.copy(balance = a.balance - amount) }, AddressCacheData(0L, 0L))
        .flatMap(_ => updateAsync(dst.hash, { a => a.copy(balance = a.balance + amount) }, AddressCacheData(amount, 0L)))
    }

  def transferSnapshot(src: Address, dst: Address, amount: Long): IO[AddressCacheData] =
    AddressService.locks.acquire(List(src.hash, dst.hash)) {
      updateAsync(src.hash, { a => a.copy(balanceByLatestSnapshot = a.balanceByLatestSnapshot - amount) }, AddressCacheData(0L, 0L))
        .flatMap(_ => updateAsync(dst.hash, { a => a.copy(balanceByLatestSnapshot = a.balanceByLatestSnapshot + amount) }, AddressCacheData(amount, 0L)))
    }
}

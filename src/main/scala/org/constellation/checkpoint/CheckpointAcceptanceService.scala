...
class CheckpointAcceptanceService[F[_]: Concurrent: Timer](
  ...
  def accept(checkpoint: CheckpointCache, facilitators: Set[Id] = Set.empty)(implicit cs: ContextShift[F]): F[Unit] = {
    ...
    val acceptCheckpoint: F[Unit] =
      for {
        ...
        blacklistedTxs <- CheckpointBlockBlacklistedAddressChecker.check(cb)(blacklistedAddresses)
        _ <- acceptTransactions(cb, Some(checkpoint), blacklistedTxs.map(_.baseHash))
        ...
        allowedToAccept <- awaitingBlocks.toList.filterA { c =>
          isAllowedToAccept(cb, blacklistedTxs)
        }
        ...
        _ <- blacklistedAddresses.addAll(doubleSpendTxs.map(_.src.address))
      } yield ()
  ...
  private def isAllowedToAccept(cb: CheckpointBlock, blockedTransactions: List[Transaction]): F[Boolean] =
    for {
      refAccepted <- AwaitingCheckpointBlock.areReferencesAccepted(transactionService.transactionChainService)(cb)
      soeAccepted <- AwaitingCheckpointBlock.areParentsSOEAccepted(checkpointParentService.soeService)(cb)
    } yield refAccepted && soeAccepted && blockedTransactions.isEmpty
  ...
  def acceptTransactions(
    ...
    cb.transactions.toList
      .filterNot(h => txsHashToFilter.contains(h.baseHash))
  ...
  private def transfer(tx: Transaction): F[Unit] =
    shouldBlockTransaction(tx).ifM(
      logger.info(s"Transaction with hash blocked = ${tx.baseHash} : is dummy = ${tx.isDummy}"),
      LiftIO[F].liftIO(dao.addressService.transfer(tx).void)
    )

  private def shouldBlockTransaction(tx: Transaction): F[Boolean] =
    for {
      isBlacklisted <- isBlacklistedAddress(tx)
      _ <- if (isBlacklisted)
        logger.info(s"Transaction blocked with hash = ${tx.baseHash} : from address = ${tx.src.address}")
      else Sync[F].unit
      _ <- if (isBlacklisted) dao.metrics.incrementMetricAsync[F]("blockedBlacklistedTxs") else Sync[F].unit

      isDummy <- isDummyTransaction(tx)
  ...
  private def isDummyTransaction(tx: Transaction): F[Boolean] =
    tx.isDummy.pure[F]

...
class UIEndpoints[F[_]](unboundedExecutionContext: ExecutionContext)(implicit F: Concurrent[F], C: ContextShift[F])
    extends Http4sDsl[F] {

  val logger = Slf4jLogger.getLogger[F]
  val blockingEc = Blocker.liftExecutionContext(unboundedExecutionContext)
...
object UIEndpoints {
  ...
  def ownerEndpoints[F[_]: Concurrent: ContextShift](
    ...
    new UIEndpoints[F](unboundedExecutionContext).ownerEndpoints(messageService)
}

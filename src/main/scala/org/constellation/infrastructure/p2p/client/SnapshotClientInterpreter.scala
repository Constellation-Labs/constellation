...
class SnapshotClientInterpreter[F[_]: Concurrent](client: Client[F]) extends SnapshotClientAlgebra[F] {
  ...
  def getStoredSnapshot(hash: String): PeerResponse[F, Array[Byte]] =
    PeerResponse[F, Vector[Byte]](s"snapshot/stored/$hash", GET) { req =>
      client.get(req.uri)(_.body.compile.toVector)
    }.map(_.toArray)
  ...
  def getSnapshotInfo(): PeerResponse[F, Array[Byte]] = // TODO: 45s timeout
    PeerResponse[F, Vector[Byte]](s"snapshot/info", GET) { req =>
      client.get(req.uri)(_.body.compile.toVector)
    }.map(_.toArray)

  def getSnapshotInfo(hash: String): PeerResponse[F, Array[Byte]] =
    PeerResponse[F, Vector[Byte]](s"snapshot/info/$hash", GET) { req =>
      client.get(req.uri)(_.body.compile.toVector)
    }.map(_.toArray)
...
object SnapshotClientInterpreter {
  ...
  def apply[F[_]: Concurrent](client: Client[F]): SnapshotClientInterpreter[F] =

...
class DownloadSpec
  ...
  "restarting a node and joining cluster" in {
    ...
    eventually(Timeout(15 minutes)) {
      val lmh = getLatestMajorityHeight
      lmh.lowest.toInt should be > 0
      lmh.highest.toInt should be > 0
    }
  ...
  private def getLatestMajorityHeight: LatestMajorityHeight =
    evalRequest[LatestMajorityHeight](GET(Uri.unsafeFromString(s"http://$instanceIp:9000/latestMajorityHeight")))
